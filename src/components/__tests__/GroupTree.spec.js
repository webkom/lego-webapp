...
describe('components', () => {
  describe('GroupTree', () => {
    it('should render the child nodes as links', () => {
      ...
      expect(wrapper.contains(<Link to='/admin/groups/2/settings'>Dog</Link>)).toBe(true);
      expect(wrapper.contains(<Link to='/admin/groups/3/settings'>Bird</Link>)).toBe(true);
    });
    ...
    it('should render the root nodes correctly', () => {
      ...
      expect(children.at(2).is('div')).toBe(true);
    });

    it('should work with no groups', () => {
      const children = shallow(<GroupTree groups={[]} />).children();
      expect(children.contains(<h3>Groups</h3>)).toBe(true);
    });
    ...
    it('should work with only root groups', () => {
      ...
      expect(wrapper.find(TreeView).isEmpty()).toBe(true);
