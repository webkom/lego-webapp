...
const renderPools = ({ fields, startTime, eventStatusType }: poolProps) => (
  ...
  >
    {fields.map((pool, index) => {
      const registrations = fields.value[index].registrations ?? [];
      const registrationStarted = registrations.length > 0;
        ...
        >
          ...
          >
            <Field
              ...
              isMulti
              SuggestionComponent={registrationStarted ? undefined : PoolSuggestion}
            />
          </Tooltip>
          {['NORMAL'].includes(eventStatusType) && (
