...
const generateCompanyProfile = (): CompanyProfile => {
  ...
  const activeListings: JobListing[] = [];
  const expiredListings: JobListing[] = [];

  if (hasActiveJobs) {
    const count = Math.floor(Math.random() * 2) + 1;
    for (let i = 0; i < count; i++) {
      activeListings.push({
        id: `active-${i}-${Math.random()}`,
  ...
  if (hasExpiredJobs) {
    ...
    for (let i = 0; i < count; i++) {
      expiredListings.push({
        id: `expired-${i}-${Math.random()}`,
        type: JOB_TYPES[Math.floor(Math.random() * JOB_TYPES.length)],
        date: getDateDaysAgo(300 + 60),
      });
    }
  }
...
const JobListingPage = () => {
  ...
  const filteredCompanies: CompanyWithStats[] = useMemo(() => {
    return companies
      .map((company) => ({
        ...company,
      ...
      render: (_, row) => (
        <Flex wrap gap="5px">
          {row.activeListings.map((job) => (
            <Tooltip
              key={job.id}
              content={`Publisert: ${job.date.toLocaleDateString()}`}
            >
              <span
          ...
          {row.expiredListings.map((job) => (
            <Tooltip
              key={job.id}
              content={`Utløp: ${job.date.toLocaleDateString()}`}
            >
              <span
