...
import styles from './NewCompactEvents.module.css';
      ...
      .map((event, key) => (
        ...
        >
          ...
          >
            ...
            <Flex  alignItems="center" className={styles.lineAndCircles}>
                <CircleLine
                    size="var(--font-size-xs)"
                    color={colorForEventType(event.eventType)}
                    length={(moment(event.endTime).diff(moment(), 'days'))}
                    /> 
            </Flex>
            <Flex alignItems="center" gap="var(--spacing-xs)" justifyContent='flex-end' className={styles.timeAndPin}>
              {event.pinned && (
