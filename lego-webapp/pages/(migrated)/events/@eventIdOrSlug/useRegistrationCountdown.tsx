...
import { registrationActionUnavailable } from '~/pages/(migrated)/events/utils';
...
export const useRegistrationCountdown = (
  ...
  const [registrationOpensIn, setRegistrationOpensIn] =
    useState<Duration | null>(null); // start as null to avoid hydration mismatch
  ...
  useEffect(() => {
    ...
    const interval = setInterval(updateState, pollingFrequency);
  ...
  return {
    ...
    registrationOpensIn:
      registrationOpensIn && registrationOpensIn.asMilliseconds() > 0
        ? registrationOpensIn
        : null,
  };
};
