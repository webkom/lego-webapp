...
const getCachedRequest = (state: Record<string, any>, endpoint: string, paginationKey: string, cursor: string, cacheSeconds: number): PromiseAction<any> | null => {
  ...
  const shouldReturnCache = Date.now() - historyObject.timestamp < cacheSeconds * 1000;
