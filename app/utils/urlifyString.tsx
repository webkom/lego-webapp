...
export const emailToLink = (mail: string): React.ReactNode => <a key={mail} href={`mailto:${mail}`} rel="noopener noreferrer" target="_blank">
    {mail}
  </a>;
export const urlToLink = (url: string): React.ReactNode => <a key={url} href={url} rel="noopener noreferrer" target="_blank">
    {url}
  </a>;

const urlifyString = (data: string): Urlified => compact(data.split(/([ \t\n])/).reduce((accumulator: Urlified, value: string, index: number, {
  length
}: Array<string>) => {
