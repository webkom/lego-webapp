export const applySelectedTheme = theme => {
  if (__CLIENT__) {
    document.documentElement.setAttribute('data-theme', theme === 'auto' ? getOSTheme() : theme);
    global.dispatchEvent(new Event('themeChange'));
  }
};
export const getTheme = () => __CLIENT__ ? document.documentElement.getAttribute('data-theme') : 'light';
