...
import { // $FlowFixMe
legacy_createStore as createStore, applyMiddleware, compose } from "redux";
...
export default function configureStore(initialState: State | $Shape<{}> = {}, {
  Sentry,
  getCookie
}: {
  ...
  const messageMiddleware = createMessageMiddleware(message => addToast({
    message
  }), Sentry);
  const middlewares = [routerMiddleware(history), thunkMiddleware.withExtraArgument({
    getCookie
  }), promiseMiddleware(), Sentry && createSentryMiddleware(Sentry, sentryMiddlewareOptions), messageMiddleware].filter(Boolean);
