...
describe('getCachedRequest', () => {
  ...
  it('should return null without endpoint', () => {
    ...
    const paginationKey = '';
    const cursor = '';
    expect(getCachedRequest(state, endpoint, paginationKey, cursor, 10)).toBeNull();
  });
  it('should return null without cacheSeconds', () => {
    ...
    const paginationKey = '';
    const cursor = '';
    expect(getCachedRequest(state, endpoint, paginationKey, cursor, null)).toBeNull();
  });
  ...
  it('should return action when endpoint exists', () => {
    ...
    const cursor = '';
    const endpoint = '/events/1';
    expect(getCachedRequest(state, endpoint, paginationKey, cursor, 10)).toEqual(action);
  });
