...
const CommentForm = ({
  ...
  return <LegoFinalForm initialValues={{
    text: EDITOR_EMPTY,
    commentKey: Math.random()
  }} validate={validate} onSubmit={({
    text
  }) => {
    return dispatch(addComment({
      ...
      return <form onSubmit={handleSubmit} className={cx(className, isOpen && styles.activeForm)}>
            ...
            <div className={cx(styles.fields, isOpen && styles.activeFields)}>
              {editorSsrDisabled ? <DisplayContent id="comment-text" className={styles.text} content="" placeholder="Skriv en kommentar" /> : <Field key={values.commentKey} autoFocus={autoFocus} name="text" placeholder="Skriv en kommentar" component={EditorField.AutoInitialized} simple />}

              {isOpen && <Button submit disabled={pristine || submitting} className={styles.submit}>
                  {submitText}
                </Button>}
              <SubmissionError />
            </div>
