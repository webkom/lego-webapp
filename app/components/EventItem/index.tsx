...
const TimeStamp = ({
  ...
  return <div className={styles.eventTime}>
      {registration && <span>
          {registration}
          <br />
        </span>}
      ...
      return <div style={{
        borderColor: colorForEvent(event.eventType)
          ...
          <Flex className={styles.companyLogoExtraCompact}>
            {event.cover && <Image src={event.cover} placeholder={event.coverPlaceholder} />}
          </Flex>
        </div>;
        ...
        borderColor: colorForEvent(event.eventType)
          ...
          <div>
            <Link to={`/events/${event.id}`}>
              ...
              {event.totalCapacity > 0 && <Attendance registrationCount={event.registrationCount} totalCapacity={event.totalCapacity} event={event} />}
            </Link>
            <TimeStamp event={event} field={field} loggedIn={loggedIn} />
            {showTags && <Flex wrap>
                {event.tags.map((tag, index) => <Tag key={index} tag={tag} />)}
              </Flex>}
          </div>
