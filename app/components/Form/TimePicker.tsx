...
function TimePickerInput({
  ...
  return <div className={styles.timePickerInput}>
...
class TimePicker extends Component<Props, State> {
  ...
  onChange = (unit: "hour" | "minute") => (e: React.SyntheticEvent<HTMLInputElement>) => {
    const {
      value
    } = e.target;

    if (unit === 'hour' && Number(value) < 24 || unit === 'minute' && Number(value) < 60) {
      this.setState(prevState => ({
  ...
  render() {
    return <div className={styles.timePicker}>
        <TimePickerInput onNext={this.onNextHour} onPrev={this.onPrevHour} value={this.state.fieldValue.hour} maxLength={2} onChange={this.onChangeHour} tabIndex={1} />
        {':'}
        <TimePickerInput onNext={this.onNextMinute} onPrev={this.onPrevMinute} value={this.state.fieldValue.minute} maxLength={2} onChange={this.onChangeMinute} tabIndex={2} />
      </div>;
  }
