...
const Tab = ({
  ...
  activePoolIndex,
  togglePool
}: any) => <Button flat onClick={() => togglePool(index)} className={cx(styles.navButton, activePoolIndex === index && styles.activeItem)}>
    {name}
  </Button>;
...
class AttendanceModal extends Component<Props, State> {
  ...
  generateAmendedPools = (pools: Array<Pool>, allRegistrations?: Array<Record<string, any>>) => {
    if (pools.length === 1) return this.setState({
      pools
    });
  ...
  render() {
    ...
    return <div>
        ...
        <ul className={styles.list}>
          {activePool.registrations.map((registration, i) => <li key={registration.id}>
        ...
        <div className={styles.nav}>
          {pools.map((pool, i) => <Tab name={pool.name} key={i} index={i} activePoolIndex={selectedPool} togglePool={togglePool} />)}
        </div>
      </div>;
  }
