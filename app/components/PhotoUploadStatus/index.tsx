...
import type { UploadStatus, GalleryPictureEntity } from "app/reducers/galleryPictures";
import { selectGalleryPictureById, initialUploadStatus } from "app/reducers/galleryPictures";
...
const UploadStatusCard = ({
  ...
  const uploadDone = uploadStatus.successCount + uploadStatus.failCount === uploadStatus.imageCount;
  const hasFailedUploads = uploadStatus.failCount !== 0;
  const word = uploadStatus.successCount === 1 ? 'bilde' : 'bilder';
  return <Card className={styles.photoUploadStatus}>
      ...
      {hasFailedUploads && <Tooltip list content={<Flex column>
              {uploadStatus.failedImages.map((name, index) => // Since we never remove elements from the list, we can use
      ...
      {lastImage && <Image alt="Last" style={{
      width: 250,
      height: 100,
      objectFit: 'cover'
...
const mapStateToProps: (arg0: any) => StateProps = state => {
  ...
  const lastImage: GalleryPictureEntity | null | undefined = selectGalleryPictureById(state, {
...
const mapDispatchToProps = dispatch => ({
  hideUploadStatus: () => dispatch({
    type: Gallery.HIDE_UPLOAD_STATUS
  })
});

const ConnectedUploadStatusCard = connect(mapStateToProps, mapDispatchToProps)(UploadStatusCard);
