...
function SearchResult({
  ...
  return <Flex wrap style={{
    ...
    borderColor: result.color
        ...
        onSelect(result);
          ...
          <h3 className={styles.searchResultTitle}>
            <span>
              ...
              {typeof result.username === 'string' && <span>({result.username})</span>}
            </span>

            {
            /* $FlowFixMe*/
          }
            {result.profilePicture ? <ProfilePicture className={styles.searchResultItemIcon} size={24} user={result} /> : <Icon name={result.icon} className={styles.searchResultItemIcon} />}
          </h3>
        </Link>
        <div>
          {result.content && <div className={styles.content}>
              <span>
      ...
      {result.picture && result.picture !== 'cover' && <Flex className={styles.picture}>
...
function SearchPageResults({
  ...
  if (results.length === 0) {
    return <EmptyState icon="glasses-outline">
        <h1>
          {query ? <span>
            ...
            </span> : <span>Søk i vei!</span>}
        </h1>
      </EmptyState>;
  }

  return <div>
      {results.map((result, i) => <SearchResult key={`${result.path}-${result.value}`} onSelect={onSelect} result={result} isSelected={selectedIndex === i} />)}
    </div>;
}
