...
function withAutocomplete<Props>({
  ...
  return class extends Component<InjectedProps & Props, State> {
    ...
    render() {
      ...
      return (// $FlowFixMe
        <WrappedComponent {...restProps} options={this.state.result} onSearch={debounce(query => this.handleSearch(query, filter), 100)} fetching={this.state.searching} filterOption={() => true} />
      );
    }
