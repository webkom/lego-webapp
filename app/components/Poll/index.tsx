...
class Poll extends Component<Props, State> {
  ...
  optionsWithPerfectRatios = (options: Array<OptionEntity>) => {
    ...
    const ratios = options.map(option => {
      return { ...option,
        ratio: option.votes / totalVotes * 100
      };
    });
  ...
  perfectRatios = (options: ReadonlyArray<OptionEntityRatio>): OptionEntityRatio[] => {
    const off = 100 - options.reduce((a, option) => a + Math.floor(option.ratio), 0);
    return sortBy<OptionEntityRatio>(options, (o: OptionEntityRatio) => Math.floor(o.ratio) - o.ratio).map((option: OptionEntityRatio, index: number) => {
      return { ...option,
  ...
  render() {
    ...
    return <div className={cx(styles.poll, backgroundLight ? styles.pollLight : '')}>
        ...
        {hasAnswered && !showResults && <div className={styles.answered}>
            Du har svart
            <Icon name="checkmark-circle-outline" size={20} style={{
          marginLeft: '10px',
          color: 'green'
        }} />
          </div>}
        {hasAnswered && showResults && <Flex column className={styles.optionWrapper}>
            <table className={styles.pollTable}>
              ...
              return <tr key={id}>
                      ...
                      <td className={styles.graphColumn}>
                        {votes === 0 ? <span className={styles.noVotes}>Ingen stemmer</span> : <div className={styles.fullGraph}>
                            <div style={{
                      width: `${ratio}%`
                    }}>
        ...
        {!hasAnswered && <Flex column className={styles.optionWrapper}>
            {!expanded && <Flex className={styles.blurContainer} onClick={this.toggleTruncate}>
                <p className={styles.blurOverlay}>
                  Klikk her for å se alle alternativene.
                </p>
                <Icon name={expanded ? 'chevron-up' : 'chevron-down'} size={60} className={cx(styles.blurOverlay, styles.blurArrow)} />
              </Flex>}
            {options && optionsToShow.map(option => <Flex className={cx(styles.alignItems, expanded ? '' : styles.blurEffect)} key={option.id}>
                  <Button dark onClick={() => handleVote(poll.id, option.id)} className={styles.voteButton}>
                    {option.name}
                  </Button>
                </Flex>)}
          </Flex>}
        <div>
          <div className={styles.moreOptionsLink}>
            {truncateOptions && (!hasAnswered || !resultsHidden || allowedToViewHiddenResults) && <div className={styles.alignItems}>
                  <Icon onClick={this.toggleTruncate} name={expanded ? 'chevron-up' : 'chevron-down'} size={20} className={styles.arrow} />
                </div>}
          </div>
          <div className={styles.bottomInfo}>
            <span>{`Stemmer: ${totalVotes}`}</span>
            {hasAnswered && !showResults && <span className={styles.resultsHidden}>
