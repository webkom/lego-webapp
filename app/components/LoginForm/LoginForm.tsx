...
class LoginForm extends Component<Props> {
  ...
  login = values => {
    ...
    return this.props.login(username, password).then(postLoginSuccess, postLoginFail).catch(err => {
  ...
  render() {
    ...
    return <Form onSubmit={handleSubmit(this.login)} className={cx(this.props.className)} onClick={e => e.stopPropagation()}>
        <Field name="username" placeholder="Brukernavn" fieldStyle={style} showErrors={false} autocomplete="username" inputRef={node => {
        this.usernameRef = node;
      }} component={TextInput.Field} />
        <Field name="password" type="password" autocomplete="current-password" placeholder="Passord" fieldStyle={style} showErrors={false} inputRef={node => {
