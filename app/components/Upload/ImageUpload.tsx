...
const FilePreview = ({
  ...
  return <Flex wrap className={styles.previewRow} alignItems="center" justifyContent="space-between">
      <div style={{
      width: '90%',
      display: 'flex'
    }}>
        ...
        flex: '1 1 auto'
          ...
          <TextInput disabled value={file.name} style={{
          width: '100%',
          height: 50
        }} />
        </div>
      </div>
      <Icon onClick={onRemove} name="trash" size={32} className={styles.removeIcon} />
    </Flex>;
};
...
const UploadArea = ({
  ...
  const onDropCallback = useCallback((files: Array<DropFile>) => {
  ...
  return <div onClick={e => {
    ...
    e.preventDefault();
      ...
      className: styles.dropArea
        ...
        {image && <Image alt="presentation" className={styles.image} src={image} />}
        <input {...getInputProps()} />
      </div>
    </div>;
};
...
export default class ImageUpload extends Component<Props, State> {
  ...
  render() {
    ...
    return <div className={styles.container}>
        {!inModal && <UploadArea onDrop={this.onDrop} multiple={multiple} image={this.state.img} />}
        <Modal contentClassName={styles.modal} show={cropOpen} onHide={this.closeModal}>
          <Fragment>
            {inModal && !preview && <div className={styles.inModalUpload}>
                <UploadArea onDrop={this.onDrop} multiple={multiple} image={this.state.img} />
              </div>}
            {preview && <Cropper ref={node => {
            this.crop = node;
          }} src={preview} className={styles.cropper} aspectRatio={aspectRatio} guides={false} />}
            {multiple && !crop && <Flex wrap column>
                {files.map((file, index) => <FilePreview onRemove={() => this.onRemove(index)} file={file} key={file.name} />)}
              </Flex>}
            <Flex wrap className={styles.footer} alignItems="center" justifyContent="space-evenly">
