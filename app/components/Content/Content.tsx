...
function Content({
  ...
  return <Flex column alignItems="center">
      {!isEmpty(youtubeParams) ? <div>
          <LoadingIndicator loading={isLoading} className={isLoading ? {} : styles.hidden} />
        ...
        maxHeight: isLoading ? '0' : isClicked ? '619px' : '358px'
            ...
            <Youtube videoId={youtubeParams.v} opts={{
          playerVars: {
            start: youtubeParams.t
          }
        }} onStateChange={() => click(true)} onReady={() => setIsLoading(false)} />
          </Flex>
        </div> : banner && <div className={cx(styles.cover, className)}>
            <Image src={banner} placeholder={bannerPlaceholder} width={1667} height={500} />
          </div>}
