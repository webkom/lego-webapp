...
export const selectEmojis = createSelector(state => state.emojis.byId, state => state.emojis.items, (emojisById, emojiIds) => {
  return emojiIds.map(id => emojisById[id]);
});
export const selectEmojisById = createSelector(selectEmojis, (state, emojisId) => emojisId, (emojis, emojisId) => {
