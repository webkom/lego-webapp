...
type State = {};
const initialState = {};
export const fetchHistoryEntryKey = (meta: {
  paginationKey: string;
  cursor: string;
  endpoint: string;
}) => // Add random (but stable) string before endpoint so that we don't mix actions with paginationKey and those without
meta && (meta.paginationKey ? 'paginationKeyAndCursor:' + meta.paginationKey + '&cursor=' + meta.cursor : meta.endpoint);
        ...
        if (__CLIENT__) {
          if (state[key]) {
            const newState = { ...state,
              [key]: { ...state[key],
                cacheCounter: state[key].cacheCounter + 1
              }
            };
