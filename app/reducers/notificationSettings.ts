...
const notificationSettings = produce<State>((newState: State, action: any): void => {
  switch (action.type) {
...
export const transform = (settings: any) => keyBy<any, string>(settings, 'notificationType');
export const selectNotificationSettingsAlternatives = (state: Record<string, any>) => ({
  channels: state.notificationSettings.channels,
  notificationTypes: state.notificationSettings.notificationTypes
});
export const selectNotificationSettings = (state: Record<string, any>) => state.notificationSettings.settings || {};