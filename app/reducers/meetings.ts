...
export const selectMeetings = createSelector(state => state.meetings.byId, state => state.meetings.items, (meetingsById, meetingIds) => meetingIds.map(id => meetingsById[id]));
export const selectMeetingById = createSelector(state => state.meetings.byId, (state, props) => props.meetingId, (meetingsById, meetingId) => meetingsById[meetingId]);
export const selectCommentsForMeeting = createSelector(selectMeetingById, state => state.comments.byId, (meeting, commentsById) => {
  if (!meeting || !meeting.comments) return [];
  return meeting.comments.map(commentId => commentsById[commentId]);
});
export const selectGroupedMeetings = createSelector(selectMeetings, meetings => {
  ...
  const oldMeetings = Object.keys(fields).map(key => ({
    title: key,
    meetings: fields[key].meetings.sort((elem1, elem2) => moment(elem2.startTime) - moment(elem1.startTime))
  })).sort((elem1, elem2) => {
  ...
  return pools.map(pool => ({
    title: pool.title,
    meetings: pool.meetings.sort((elem1, elem2) => Number(moment(elem1.startTime)) - Number(moment(elem2.startTime)))
  })).concat(oldMeetings).filter(elem => elem.meetings.length);
});