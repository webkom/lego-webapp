  ...
  mutate: produce((newState: State, action: any): void => {
    switch (action.type) {
      case Event.SOCKET_EVENT_UPDATED:
        {
          const registrations = normalize(action.payload, eventSchema).entities.registrations || {};
          newState.byId = mergeObjects(newState.byId, registrations);
          newState.items = union(newState.items, Object.keys(registrations).map(Number));
          break;
        }
      ...
      case Event.SOCKET_PAYMENT.FAILURE:
        {
          const registration = normalize(action.payload, registrationSchema).entities.registrations[action.payload.id];

          if (!registration) {
            return;
          }

          newState.byId[registration.id] = { ...omit(newState.byId[registration.id], 'unregistrationDate'),
            ...registration
          };
      ...
      case Event.REQUEST_UNREGISTER.SUCCESS:
        {
          const registrations = normalize(action.payload, registrationSchema).entities.registrations;
          newState.byId = mergeObjects(newState.byId, registrations);
      ...
      case Event.SOCKET_PAYMENT.SUCCESS:
        {
          ...
          newState.byId[registration.id] = { ...omit(newState.byId[registration.byId], ['unregistrationDate', 'clientSecret']),
            ...registration
          };
          break;
        }
      ...
      case Event.UPDATE_REGISTRATION.SUCCESS:
        {
          const registration = normalize(action.payload, registrationSchema).entities.registrations[action.payload.id];
          newState.byId[action.payload.id] = { ...newState.byId[action.payload.id],
            ...registration
          };
          break;
        }
      ...
      case Event.SOCKET_UNREGISTRATION.SUCCESS:
        {
          ...
          const registrations = normalize(transformedPayload, registrationSchema).entities.registrations;
          newState.byId = mergeObjects(newState.byId, registrations);
          newState.items = union(newState.items, [action.payload.id]);
          break;
        }
