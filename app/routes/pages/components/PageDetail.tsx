...
class PageDetail extends Component<Props, State> {
  ...
  render() {
    ...
    return <Content className={styles.cont}>
        ...
        <div className={styles.main}>
          ...
          <Flex className={styles.page}>
            <Sidebar categorySelected={category} currentUrl={currentUrl} pageHierarchy={pageHierarchy} isOpen={this.state.isOpen} handleClose={this.closeSidebar} />

            <div className={styles.mainTxt}>
              <NavigationTab className={styles.navTab}>
                {actionGrant.includes('edit') && editUrl && <NavigationLink to={editUrl}>Rediger</NavigationLink>}
                {actionGrant.includes('create') && <NavigationLink to="/pages/new">Lag ny</NavigationLink>}
              </NavigationTab>

              {isComplete ? <MainPageRenderer page={selectedPage} pageInfo={selectedPageInfo} ChildPageRenderer={PageRenderer} loggedIn={loggedIn} /> : <LoadingIndicator loading />}
            </div>
          </Flex>
        </div>
      </Content>;
...
export const MainPageRenderer = ({
  ...
  return <article className={styles.pageWrapper}>
      <div className={styles.headWrapper}>
        {pageBanner && <div className={styles.banner}>
            <Image alt={`${title} page banner`} src={pageBanner} placeholder={pageBannerPlaceholder} />
          </div>}
        {title !== 'Info om Abakus' && <h1 className={styles.header1}>{readmeIfy(title)}</h1>}
      </div>
      <ChildPageRenderer page={page} pageInfo={pageInfo} loggedIn={loggedIn} />
    </article>;
};
export const FlatpageRenderer = ({
  page
}: {
  page: PageEntity;
  ...
  return <article className={styles.detail}>
      ...
      <div className={styles.membersSection}>
        <div className={styles.leaderBoard}>
          {leaders.map(({
          user
        }, key) => <GroupMember user={user} key={user.id} leader groupName={name} />)}
          {co_leaders.map(({
          user
        }, key) => <GroupMember user={user} key={user.id} co_leader />)}
        </div>
        <div className={styles.members}>
          {members.map(({
          user
        }, key) => <GroupMember user={user} key={user.id} />)}
        </div>
        <div className={styles.members}>
          {activeRetirees.map(({
          user
