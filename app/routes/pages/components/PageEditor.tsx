...
import { EditorField, TextInput, Form, withSubmissionError, SelectInput, ObjectPermissions } from "app/components/Form";
...
export default class PageEditor extends Component<Props, State> {
  ...
  onSubmit = (data: {
    ...
    const body = { ...normalizeObjectPermissions(data),
      title: data.title,
  ...
  render() {
    ...
    const backUrl = isNew ? '/pages/info-om-abakus' : `/pages/${page.category}/${pageSlug}`;
    return <Content>
        <NavigationTab title={page.title} back={{
        label: 'Tilbake',
        path: backUrl
      }} />
        <Form onSubmit={handleSubmit(withSubmissionError(this.onSubmit))}>
          <div className={styles.coverImage}>
            <ImageUpload aspectRatio={20 / 6} onSubmit={this.setPicture} img={images[this.state.page.picture] ? images[this.state.page.picture] : page.picture} />
          </div>

          <Flex justifyContent="space-between">
            <Field placeholder="Title" name="title" component={TextInput.Field} id="page-title" />
            <Field name="category" component={SelectInput.Field} placeholder="Velg kategori" options={categoryOptions} />

            <Flex margin="0 0 0 10px">
              {!isNew && <ConfirmModalWithParent title="Slett side" message="Er du sikker på at du vil slette denne infosiden?" onConfirm={this.onDelete}>
                  <Button danger>
          ...
          <Field placeholder="Write page content here..." name="content" component={EditorField.Field} uploadFile={uploadFile} initialized={this.props.initialized || isNew} />
        </Form>
      </Content>;
  }
