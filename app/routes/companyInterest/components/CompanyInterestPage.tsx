...
import { TextEditor, TextInput, Button, CheckBox, SelectInput, Form, RadioButton, RadioButtonGroup } from "app/components/Form";
...
const eventToString = event => Object.keys(EVENT_TYPES)[Number(event.charAt(event.length - 2))];

const otherOffersToString = offer => Object.keys(OTHER_TYPES)[Number(offer.charAt(offer.length - 2))];

const collaborationToString = collab => Object.keys(COLLABORATION_TYPES)[Number(collab.charAt(collab.length - 2))];

const targetGradeToString = targetGrade => Object.keys(TARGET_GRADE_TYPES)[Number(targetGrade.charAt(targetGrade.length - 2))];
...
const SemesterBox = ({
  ...
  language: string;
} & FieldArrayProps): Node => <Flex column className={styles.checkboxWrapper}>
    {fields.map((item, index) => <Flex key={index}>
        <label className={styles.checkboxLabel}>
          <div className={styles.checkboxField}>
            <Field key={`semester${index}`} name={`semesters[${index}].checked`} component={CheckBox.Field} normalize={v => !!v} />
          </div>
...
const EventBox = ({
  ...
  language: string;
    ...
    {fields.map((key, index) => <Flex key={index}>
        <label className={styles.checkboxLabel}>
          <div className={styles.checkboxField}>
            <Field key={`events[${index}]`} name={`events[${index}].checked`} component={CheckBox.Field} normalize={v => !!v} />
          </div>
          <span className={styles.checkboxSpan}>
...
const TargetGradeBox = ({
  ...
  language: string;
    ...
    {fields.map((key, index) => <Flex key={index}>
        <label className={styles.checkboxLabel}>
          <div className={styles.checkboxField}>
            <Field key={`targetGrades[${index}]`} name={`targetGrades[${index}].checked`} component={CheckBox.Field} normalize={v => !!v} />
          </div>
          <span className={styles.checkboxSpan}>
...
const OtherBox = ({
  ...
  language: string;
    ...
    {fields.map((key, index) => <Flex key={index}>
        <label className={styles.checkboxLabel}>
          <div className={styles.checkboxField}>
            <Field key={`otherOffers[${index}]`} name={`otherOffers[${index}].checked`} component={CheckBox.Field} normalize={v => !!v} />
          </div>
          <span className={styles.checkboxSpan}>
...
const CollaborationBox = ({
  ...
  language: string;
    ...
    {fields.map((key, index) => <Flex key={index}>
        <label className={styles.checkboxLabel}>
          <div className={styles.checkboxField}>
            <Field key={`collaborations[${index}]`} name={`collaborations[${index}].checked`} component={CheckBox.Field} normalize={v => !!v} />
          </div>
          <span className={styles.checkboxSpan}>
...
type Props = FormProps & {
  allowedBdb: boolean;
  onSubmit: (arg0: CompanyInterestFormEntity, arg1: boolean | null | undefined) => Promise<any>;
...
const CompanyInterestPage = (props: Props) => {
  ...
  const onSubmit = data => {
    ...
    const [range_start, range_end] = data.participantRange ? PARTICIPANT_RANGE_MAP[data.participantRange] : [null, null];
    const newData = {
      ...
      mail: data.mail,
      phone: data.phone,
      semesters: data.semesters.filter(semester => semester.checked).map(semester => semester.id),
      events: data.events.filter(event => event.checked).map(event => event.name),
      otherOffers: data.otherOffers.filter(offer => offer.checked).map(offer => offer.name),
      collaborations: data.collaborations.filter(collab => collab.checked).map(collab => collab.name),
      targetGrades: data.targetGrades.filter(targetGrade => targetGrade.checked).map(targetGrade => Number(targetGrade.name)),
    ...
    return props.onSubmit(newData, isEnglish).then(() => props.push(props.allowedBdb ? '/companyInterest/' : '/pages/bedrifter/for-bedrifter')).catch(err => {
  ...
  const showCourseComment = props.interestForm.events?.some(e => e.name === 'course' && e.checked === true);
  const showBreakfastTalkComment = props.interestForm.events?.some(e => e.name === 'breakfast_talk' && e.checked === true);
  const showOtherEventComment = props.interestForm.events?.some(e => e.name === 'other' && e.checked === true);
  return <Content>
      <Form onSubmit={props.handleSubmit(onSubmit)}>
        <FlexRow alignItems="center" justifyContent="space-between">
          <h1 className={styles.mainHeading}>{labels.mainHeading[language]}</h1>
          <Link to={isEnglish ? '/interesse' : '/register-interest'} style={{
        ...
        <Field name="company" label={labels.company.header[language]} placeholder={labels.company.placeholder[language]} filter={['companies.company']} fieldClassName={styles.metaField} component={withAutocomplete({
        WrappedComponent: SelectInput.Field,
        retainFailedQuery: true
      })} required />
        <Field label={labels.contactPerson.header[language]} placeholder={labels.contactPerson.placeholder[language]} name="contactPerson" component={TextInput.Field} required />
        <Field label={labels.mail[language]} placeholder="example@gmail.com" name="mail" component={TextInput.Field} required />
        <Field label={labels.phone[language]} placeholder="+47 909 09 090" name="phone" component={TextInput.Field} required />
        ...
        <Flex wrap justifyContent="space-between">
          <Flex column className={styles.interestBox}>
            ...
            <FieldArray label="semesters" name="semesters" language={language} component={SemesterBox} />
          </Flex>
          <Flex column className={styles.interestBox}>
            <label htmlFor="events" className={styles.heading}>
              {labels.events[language]}
            </label>
            <FieldArray name="events" language={language} component={EventBox} />
          </Flex>
          <Flex column className={styles.interestBox}>
            <label htmlFor="collaborations" className={styles.heading}>
              {labels.collaborations[language]}
            </label>
            <FieldArray name="collaborations" language={language} component={CollaborationBox} />
          </Flex>
        </Flex>
        ...
        <Flex wrap justifyContent="space-between">
          <Flex column className={styles.interestBox}>
            ...
            <FieldArray name="targetGrades" language={language} component={TargetGradeBox} />
          </Flex>
          ...
          <Flex column className={styles.interestBox}>
            ...
            <RadioButtonGroup name="participantRange">
              {Object.keys(PARTICIPANT_RANGE_TYPES).map((key, index) => <Field key={index} name={key} label={PARTICIPANT_RANGE_TYPES[key]} component={RadioButton.Field} inputValue={key} />)}
            </RadioButtonGroup>
          </Flex>
          <Flex column className={styles.interestBox}>
            ...
            <FieldArray name="otherOffers" language={language} component={OtherBox} />
          </Flex>
        </Flex>
        ...
        <Field placeholder={interestText.comment[language]} name="comment" component={TextEditor.Field} rows={10} className={styles.textEditor} label={labels.comment[language]} required />

        {showCourseComment && <div className={styles.topline}>
            <p>{interestText.courseDescription[language]}</p>
            <Field placeholder={interestText.courseComment[language]} name="courseComment" component={TextEditor.Field} rows={10} className={styles.textEditor} label={labels.secondComment[language]} required />
          </div>}

        {showBreakfastTalkComment && <div className={styles.topline}>
            <p>{interestText.breakfastTalkDescription[language]}</p>
            <Field placeholder={interestText.breakfastTalkComment[language]} name="breakfastTalkComment" component={TextEditor.Field} rows={10} className={styles.textEditor} label={labels.secondComment[language]} required />
          </div>}

        {showOtherEventComment && <div className={styles.topline}>
            <p>{interestText.otherEventDescription[language]}</p>
            <Field placeholder={interestText.otherEventComment[language]} name="otherEventComment" component={TextEditor.Field} rows={10} className={styles.textEditor} label={labels.secondComment[language]} required />
          </div>}
