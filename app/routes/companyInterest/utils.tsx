...
export const sortSemesterChronologically = (a: CompanySemesterEntity, b: CompanySemesterEntity) => {
  const semesterCodeToPriority = {
    spring: 0,
    autumn: 1
  };
  return Number(a.year) !== Number(b.year) ? Number(a.year) - Number(b.year) : semesterCodeToPriority[a.semester] - semesterCodeToPriority[b.semester];
};
export const SemesterNavigation = ({
  title
}: {
  title: Node;
...
export const semesterToText = ({
  ...
  return semester === 'spring' || semester === 'autumn' ? `${SEMESTER_TRANSLATION[semester][language]} ${year}` : '';
};
export const interestText = {
  ...
  text: {
    first: {
      norwegian: 'Dersom dere ikke har hatt arrangement med Abakus før, eller om det er lenge siden vi har samarbeidet, ønsker vi at dere skriver litt om selskapet. Dette hjelper oss mye når vi skal sette sammen arrangementskalenderen.',
      english: 'If you have not previously organized an event with Abakus previously, or if it has  been a while since we last cooperated, we would like you to tell us a bit about  your company. That is helpful when we are distributing dates for events. '
    },
    second: {
      norwegian: 'Vi ønsker også at dere skriver litt om hva slags type arrangement dere ser for dere å holde. Ønsker dere å gjøre noe utenfor de vanlige rammene, eller helst en standard bedpres? Uansett vil vi gjerne vite det!',
      english: "We'd prefer that you also write a little bit about what kind of event you would like to have. Do you want something outside the given options, or is it a regular company presentation? Either way, we'd like to know!"
    }
  },
  bedex: {
    norwegian: '«Husk å ranger datoer og gruppestørrelse dersom du har huket av for BedEx»',
    english: '«Remember to rank dates and groupsize if you have checked BedEx»'
  },
  anniversaryCollaboration: {
    norwegian: '*Samarbeid med Jubileum vil si promoteringsmuligheter på Abakus sitt jubileum i vår, eller Revyen sitt Jubileum i november 2021. Dersom det er av interesse, vil dere informeres om hva et slikt samarbeid vil innebære.',
  ...
  revueCollaboration: {
    norwegian: '**Samarbeid med Revyen innebærer at dere får holde bedriftspresentasjon på datoen for Revyen, samt gode promoteringsmuligheter (logo på revy-gensere, logo på revyplakater o.l.)',
  ...
  priorityReasoning: {
    norwegian: 'Vi velger bedrifter på bakgrunn av mange faktorer. Vi ønsker likevel å vektlegge bedrifter med ambisjoner om et alternativt opplegg. Bedrifter som for eksempel er interessert i å samarbeide med andre linjeforeninger/bedrifter om større arrangement (som Bedrift-til-Bedrift). Vi begrunner dette med et ønske fra studentene om nytenking rundt «formatet» til en standard bedriftspresentasjon. Vi tror dette vil gi studentene et attraktivt arrangement, som er mer konkurransedyktig enn det som  tilbys i dag. ',
