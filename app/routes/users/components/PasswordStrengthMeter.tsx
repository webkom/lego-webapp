...
import { passwordLabel, barColor, passwordFeedbackMessages } from "./passwordStrengthVariables";
...
const PasswordStrengthMeter = ({
  ...
  return <Zxcvbn fallback={<>
      ...
      const zxcvbnValue = zxcvbn(password, [user.username, user.firstName, user.lastName]);
      ...
      const crackTimeSec = Number(zxcvbnValue.crack_times_seconds?.offline_slow_hashing_1e4_per_second);
      const crackTimeDuration = moment.duration(crackTimeSec, 'seconds').humanize();
      const crackTime = crackTimeSec > 2 ? crackTimeDuration : crackTimeSec + ' sekunder';
      return <>
            ...
            {password && zxcvbnValue.score < 3 && <ul className={styles.tipsList}>
                {tips.map((value, key) => {
...
const PasswordStrengthBar = ({
  ...
  return <div className={styles.removeLabels}>
      <Bar labels={[1, 2, 3, 4, 5]} labelColor="#000" progress={strengthScore * 25} barColor={barColor[strengthScore]} seperatorColor="#fff" />
    </div>;
};
