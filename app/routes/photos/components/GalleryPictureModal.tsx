...
type Props = {
  ...
  hasMore: Boolean;
  fetchSiblingGallerPicture: (arg0: EntityID, arg1: EntityID, arg2: boolean) => Promise<any>;
...
const Taggees = ({
  ...
  if (taggees.length === 1) {
    ...
    return <span>
        <br />
        <span>med </span>
        {taggees.map((taggee, index) => <span key={taggee.id}>
            {taggees.length === index + 1 ? <span>
                <span>{'og '}</span>
...
export default class GalleryPictureModal extends Component<Props, State> {
  ...
  handleDelete = (clickedDeletePicture: number) => {
    if (this.state.clickedDeletePicture === clickedDeletePicture) {
      this.props.deletePicture(this.props.gallery.id, this.props.picture.id).then(() => this.props.push(`/photos/${this.props.gallery.id}`));
    } else {
      this.setState({
        clickedDeletePicture
      });
  ...
  siblingGalleryPicture = (next: boolean) => {
    ...
    return this.props.fetchSiblingGallerPicture(gallery.id, pictureId, next).then(result => {
      this.setState({
        hasNext: !!result.payload.next,
        hasPrevious: !!result.payload.previous
      });
      return result.payload.result.length > 0 && push(`/photos/${gallery.id}/picture/${result.payload.result[0]}`);
    });
  };
  previousGalleryPicture = throttle(() => this.siblingGalleryPicture(false), 500, {
    trailing: false
  });
  ...
  render() {
    ...
    return <Modal onHide={() => push(`/photos/${gallery.id}`)} show contentClassName={styles.content}>
        <Swipeable onSwiping={this.handleSwipe}>
          ...
          <Content className={styles.topContent}>
            <Flex width="100%" justifyContent="space-between" alignItems="center">
              <Flex justifyContent="space-between">
                <Image className={styles.galleryThumbnail} alt="some alt" src={gallery.cover.thumbnail} />
              ...
              <Dropdown show={showMore} toggle={this.toggleDropdown} closeOnContentClick className={styles.dropdown} contentClassName={styles.dropdownContent} iconName="options-outline">
                <Dropdown.List>
                  <Dropdown.ListItem>
                    <a href={picture.rawFile} download onClick={this.toggleDropdown} style={{
                  ...
                  {actionGrant && actionGrant.includes('edit') && [<Dropdown.ListItem key="edit">
                        <Link to="#" onClick={this.onUpdate} style={{
                    color: 'var(--lego-color-gray)'
                  }}>
                          <strong>Rediger</strong>
                          <Icon name="create-outline" size={24} />
                        </Link>
                      </Dropdown.ListItem>, <Dropdown.ListItem key="cover">
                        <Link onClick={this.onUpdateGalleryCover} to="#" style={{
                    color: 'var(--lego-color-gray)'
                  }}>
                          <strong>Sett som album cover</strong>
                          <Icon name="image-outline" size={24} />
                        </Link>
                      </Dropdown.ListItem>, <Dropdown.Divider key="divider" />, <Dropdown.ListItem key="delete" onClick={e => {
                  e.preventDefault();
                  e.stopPropagation();
                }}>
                        <RenderGalleryPicture handleDelete={this.handleDelete} id={pictureId} clickedDeletePicture={this.state.clickedDeletePicture} />
                      </Dropdown.ListItem>]}
                </Dropdown.List>
              </Dropdown>
            </Flex>
          ...
          <Content className={styles.bottomContent}>
            <Flex justifyContent="center">
              {this.state.hasPrevious && <Link to="#" onClick={this.previousGalleryPicture} style={{
              marginRight: '50px'
            }}>
                  <Icon name="arrow-back-circle-outline" size={64} />
                </Link>}
              {this.state.hasNext && <Link to="#" onClick={this.nextGalleryPicture}>
            ...
            <Flex className={styles.pictureDescription}>
              <p>
                {picture.description}
                {picture.taggees.length > 0 && <Taggees taggees={picture.taggees} />}
              </p>
            </Flex>
            {picture.contentTarget && <Flex className={styles.pictureDescription}>
                <CommentView style={{
              width: '100%'
            }} formEnabled user={currentUser} contentTarget={picture.contentTarget} loggedIn={loggedIn} comments={comments} deleteComment={deleteComment} />
              </Flex>}
          </Content>
        </Swipeable>
      </Modal>;
