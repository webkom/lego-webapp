...
const GalleryEditorActions = ({
  ...
  onDeletePictures,
  newPicutureStatus
}: Props) => <Collapse isOpened={selectedCount > 0}>
    <Sticky enabled={selectedCount > 0} innerZ={10} top={0}>
      <Flex className={styles.actionsContainer}>
        ...
        <div>
          {selectedCount === 1 && <Button className={styles.action} onClick={onUpdateGalleryCover}>
              Sett album cover
            </Button>}
          {newPicutureStatus !== -1 && <Button onClick={() => onTogglePicturesStatus(!!newPicutureStatus)} className={styles.action}>
