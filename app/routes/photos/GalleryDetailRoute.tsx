...
import { fetch, clear, uploadAndCreateGalleryPicture } from "app/actions/GalleryPictureActions";
...
}, dispatch) => Promise.all([dispatch(fetch(params.galleryId)).catch(), dispatch(fetchGallery(params.galleryId)).catch(err => dispatch(fetchGalleryMetadata(params.galleryId)))]);
    ...
    class MetadataHelper extends PureComponent<Props & LoginProps> {
      render() {
        ...
        if (gallery && gallery.createdAt) {
          return <ActualComponent fetching={fetching} loggedIn={loggedIn} gallery={gallery} {...props} />;
        }
        ...
        if (!loggedIn) {
          ...
          return <LoginPage gallery={gallery} loggedIn={loggedIn} fetching={fetching} {...props} />;
        }
...
export default compose(connect(mapStateToProps, mapDispatchToProps), prepare(loadData), loadingIndicator(['gallery.title']), helmet(propertyGenerator), metadataHelper())(GalleryDetail);