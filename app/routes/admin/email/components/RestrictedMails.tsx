...
export default class RestrictedMails extends Component<Props> {
  render() {
    const columns = [{
      title: 'ID',
      dataIndex: 'id',
      render: (id, restrictedMail) => <Link to={`/admin/email/restricted/${restrictedMail.id}`}>{id}</Link>
    }, {
      title: 'Fra',
      search: true,
      ...
      title: 'Brukt',
      dataIndex: 'used',
      render: used => used ? <Tag tag={moment(used).format('lll')} color="gray" /> : <Tag tag="Ubrukt" color="yellow" />
    }];
    return <div>
        <p>
        ...
        <Table infiniteScroll columns={columns} onFetch={() => {
        this.props.fetch({
          next: true
        });
      }} hasMore={this.props.hasMore} loading={this.props.fetching} data={this.props.restrictedMails} />
      </div>;
  }
