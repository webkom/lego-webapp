...
export default class EmailUsers extends Component<Props> {
  render() {
    const columns = [{
      ...
      render: (_, emailUser) => <Link to={`/admin/email/users/${emailUser.id}`}>
      ...
      search: true,
      inlineFiltering: false,
      render: (internalEmail: string, emailUser) => <Link to={`/admin/email/users/${emailUser.id}`}>
      ...
      filter: this.props.committees.map(committee => ({
      ...
      render: (_, emailUser) => {
        const output = emailUser.user.abakusGroups.filter(abakusGroup => abakusGroup.type === GroupTypeCommittee).map(committee => <Link key={committee.id} to={`/admin/groups/${committee.id}/members`}>
                {committee.name}{' '}
              </Link>);
      ...
      title: 'Klasse',
      dataIndex: 'userGrade',
      filter: this.props.grades.map(grade => ({
        label: grade.name,
        value: grade.name
      })).concat({
        label: 'Ikke student',
      ...
      render: (_, emailUser) => {
        const output = emailUser.user.abakusGroups.filter(abakusGroup => abakusGroup.type === GroupTypeGrade).map(grade => <Link key={grade.id} to={`/admin/groups/${grade.id}/members`}>
                {grade.name}{' '}
              </Link>);
      ...
      render: enabled => enabled ? <Tag tag="Aktiv" color="green" /> : <Tag tag="Inaktiv" color="gray" />
    }];
    return <div>
        <p>
      ...
      }} hasMore={this.props.hasMore} loading={this.props.fetching} data={this.props.emailUsers} />
      </div>;
  }
