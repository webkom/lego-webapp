...
import { fetchMemberships, fetchMembershipsPagination, addMember, removeMember } from "app/actions/GroupActions";
...
export const GroupMembers = ({
  ...
  query,
  filters
}: Props) => <div className={styles.groupMembers}>
    <>
      Antall medlemmer (inkl. undergrupper):{' '}
      {groupsById[groupId.toString()].numberOfUsers}
    </>
    {showDescendants || <AddGroupMember addMember={addMember} groupId={groupId} />}
    <LoadingIndicator loading={!memberships}>
      <h3 className={styles.subTitle}>Brukere</h3>
      <GroupMembersList key={groupId + showDescendants} groupId={groupId} filters={filters} query={query} hasMore={hasMore} push={push} pathname={pathname} search={search} groupsById={groupsById} fetch={fetch} fetching={fetching} showDescendants={showDescendants} removeMember={removeMember} memberships={memberships} />
    </LoadingIndicator>
  </div>;
...
export default compose(connect(mapStateToProps, mapDispatchToProps), prepare(loadData, ['match.params.groupId']))(GroupMembers);