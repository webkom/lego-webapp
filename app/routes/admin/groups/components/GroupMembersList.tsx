...
const GroupMembersList = ({
  ...
  const GroupMembersListColumns = (fullName, membership) => {
    ...
    return true && <>
          <ConfirmModalWithParent title="Bekreft utmelding" message={`Er du sikker på at du vil melde ut "${user.fullName}" fra gruppen "${groupsById[abakusGroup].name}?"`} onConfirm={() => removeMember(membership)}>
  ...
  const GroupLinkRender = abakusGroup => true && <Link to={`/admin/groups/${abakusGroup}/members?descendants=false`}>
        {groupsById[abakusGroup] && groupsById[abakusGroup].name}
      </Link>;

  const RoleRender = (role: string) => role !== 'member' && <i>{ROLES[role] || role} </i>;
  ...
  const columns = [{
    ...
    search: false,
    inlineFiltering: false,
    filterMapping: role => role === 'member' || !ROLES[role] ? '' : ROLES[role],
    render: RoleRender
  }].filter(Boolean);
  return <>
      <Table infiniteScroll onChange={(filters, sort) => {
      push({
        pathname,
        search: qs.stringify({
    ...
    }} hasMore={hasMore} loading={fetching} data={memberships} filters={filters} />
