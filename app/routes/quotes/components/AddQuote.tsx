  ...
  return <div className={styles.root}>
      ...
      <div className={styles.addQuote}>
        <form onSubmit={handleSubmit(withSubmissionError(addQuotes))}>
          <Field placeholder="Eks: Det er bare å gjøre det" label="Selve sitatet" name="text" component={TextEditor.Field} />

          <Field placeholder="Eks: Esso" label="Hvor sitatet kommer fra (sleng gjerne med noe snaks!)" name="source" component={TextEditor.Field} type="text" />
      ...
      <div className={styles.outerPreview}>
        ...
        <div className={styles.innerPreview}>
          <RandomQuote fetchRandomQuote={() => Promise.resolve()} addReaction={() => Promise.resolve()} deleteReaction={() => Promise.resolve()} fetchEmojis={() => Promise.resolve()} fetchingEmojis={false} emojis={[]} currentQuote={{
          id: 1,
          ...
          reactionsGrouped: [],
          reactions: []
        }} loggedIn={true} useReactions={false} />
        </div>
      </div>
    </div>;
};
