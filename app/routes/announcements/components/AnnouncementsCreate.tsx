...
let AnnouncementsCreate = ({
  ...
  return <ContentMain>
      ...
      {actionGrant.includes('create') && <Flex column>
          <h2 className={styles.header}>Ny kunngjøring</h2>
          <Form className={styles.form}>
            <Field className={styles.msgField} name="message" component={TextArea.Field} placeholder="Skriv din melding her..." label="Kunngjøring:" />
            <span className={styles.formHeaders}>Mottakere:</span>
            <Flex className={styles.rowRec}>
              <Field className={styles.recField} name="users" placeholder="Brukere" filter={['users.user']} isMulti component={SelectInput.AutocompleteField} />
              <Field name="groups" placeholder="Grupper" filter={['users.abakusgroup']} isMulti component={SelectInput.AutocompleteField} />
            </Flex>
            <Flex className={styles.rowRec}>
              <Field className={styles.recField} name="events" placeholder="Arrangementer" filter={['events.event']} isMulti component={SelectInput.AutocompleteField} />
              <Field name="meetings" placeholder="Møter" filter={['meetings.meeting']} isMulti component={SelectInput.AutocompleteField} />
            </Flex>
            <span className={styles.formHeaders}>Send på vegne av:</span>
            <Field name="fromGroup" placeholder="Fra gruppe" filter={['users.abakusgroup']} component={SelectInput.AutocompleteField} />
            <Flex>
              <Button onClick={handleSubmit(values => onSubmit(values, false))} disabled={disabledButton}>
                Opprett
              </Button>
              <Button onClick={handleSubmit(values => onSubmit(values, true))} disabled={disabledButton}>
                Opprett og send
              </Button>
            </Flex>
          </Form>
...
AnnouncementsCreate = connect(state => {
  ...
  return {
    initialValues: {
      groups: locationState?.group ? selectAutocomplete([{
        contentType: 'users.abakusgroup',
        ...locationState.group
      }]) : [],
      events: locationState?.event ? selectAutocomplete([{
        contentType: 'events.event',
        ...locationState.event
      }]) : [],
      meetings: locationState?.meeting ? selectAutocomplete([{
