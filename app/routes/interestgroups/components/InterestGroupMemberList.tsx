...
export default class InterestGroupMemberList extends Component<Props, State> {
  ...
  render() {
    const sorted = sortBy(this.props.memberships, ({
      role
    }) => SORT_ORDER.indexOf(role)).reverse();
    return <div>
        <div onClick={this.toggleModal}>{this.props.children}</div>
        <Modal show={this.state.modalVisible} onHide={this.toggleModal}>
          <h2>Medlemmer</h2>
          <ul className={styles.list}>
            {sorted.map(membership => <ListedUser key={membership.user.id} user={membership.user} role={membership.role} />)}
          </ul>
        </Modal>
      </div>;
  }
