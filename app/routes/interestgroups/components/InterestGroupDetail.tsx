...
const Title = ({
  ...
  showEdit
}: TitleProps) => <NavigationTab title={name} back={{
  label: 'Tilbake',
  path: '/interest-groups'
}}>
    {showEdit && <NavigationLink to={`/interest-groups/${id}/edit`}>
        Rediger
      </NavigationLink>}
  </NavigationTab>;
...
const Members = ({
  group,
  members
}: MembersProps) => <Flex column>
    <h4>Medlemmer {group.numberOfUsers}</h4>
    <UserGrid users={members && members.slice(0, 14).map(reg => reg.user)} maxRows={2} minRows={2} />
...
const ButtonRow = ({
  ...
  const [membership] = group.memberships.filter(m => m.user.id === currentUser.id);
  const onClick = membership ? () => leaveGroup(membership, group.id) : () => joinGroup(group.id, currentUser);
  return <Flex>
...
const Contact = ({
  ...
  if (leaders.length > 1) {
    return <Flex column>
        <h4>Ledere</h4>
        <ul>
          {leaders.map(leader => <li key={leader.user.username}>{leader.user.fullName}</li>)}
        </ul>
      </Flex>;
  }
...
function InterestGroupDetail(props: Props) {
  ...
  return <Content>
      ...
      <ContentSection>
        ...
        <ContentSidebar>
          <Image className={styles.logo} src={logo} placeholder={group.logoPlaceholder} />
