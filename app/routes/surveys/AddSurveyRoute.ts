...
import SurveyEditor, { initialQuestion } from "./components/SurveyEditor/SurveyEditor";
...
const loadData = (props, dispatch) => {
  ...
  if (event) {
    return dispatch(fetchEvent(event)).then(result => dispatch(fetchTemplate(result.payload.entities.events[event].eventType)));
  }
...
export default compose(replaceUnlessLoggedIn(LoginPage), prepare(loadData, ['location.search']), connect(mapStateToProps, mapDispatchToProps), loadingIndicator(['notFetching']))(SurveyEditor);