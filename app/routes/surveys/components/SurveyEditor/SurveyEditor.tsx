...
import { TextInput, SelectInput, DatePicker, legoForm } from "app/components/Form";
...
type Props = FormProps & {
  ...
  submitFunction: (arg0: SurveyEntity, arg1: number | null | undefined) => Promise<any>;
...
function TemplateTypeDropdownItems({
  ...
  const link = eventType => survey && survey.id ? `/surveys/${survey.id}/edit/?templateType=${eventType}` : `/surveys/add/?templateType=${eventType}`;

  return <Dropdown.List>
      {Object.keys(EVENT_CONSTANTS).map(eventType => <Dropdown.ListItem key={eventType}>
          <Link to="#" onClick={e => {
        e.preventDefault();
        e.stopPropagation();
        destroy();
...
class SurveyEditor extends Component<Props, State> {
  ...
  renderQuestions = ({
    ...
    return <>
        <ul className={styles.questions} key="questions">
          {fields.map((question, i) => <Question key={i} numberOfQuestions={fields.length} question={question} questionData={fields.get(i)} deleteQuestion={() => Promise.resolve(fields.remove(i))} updateRelativeIndexes={this.updateRelativeIndexes} relativeIndex={i} fields={fields} />)}
          <span>{error}</span>
        </ul>

        <Link key="addNew" to="#" onClick={() => {
  ...
  render() {
    ...
    const titleField = <Field placeholder="Tittel" label=" " autoFocus={autoFocus} name="title" component={TextInput.Field} className={styles.editTitle} />;
    return <Content className={styles.detail}>
        <Helmet title={survey.id ? survey.title : 'Ny spørreundersøkelse'} />
        {survey && survey.id ? <DetailNavigation title={titleField} surveyId={Number(survey.id)} /> : <ListNavigation title={titleField} />}
        <ConfirmModalWithParent title="Bekreft bruk av mal" message={'Dette vil slette alle ulagrede endringer i undersøkelsen!\n' + 'Lagrede endringer vil ikke overskrives før du trykker "Lagre".'} closeOnConfirm={true} onCancel={() => {
        this.setState(state => ({
          templatePickerOpen: false
        }));
        ...
        return Promise.resolve();
          ...
          <Button className={styles.templatePicker}>
            ...
            <Dropdown className={styles.templateDropdown} show={this.state.templatePickerOpen} toggle={() => {
            this.setState(state => ({
              templatePickerOpen: !state.templatePickerOpen
            }));
          }} closeOnContentClick>
              <TemplateTypeDropdownItems survey={survey} push={push} destroy={destroy} />
            </Dropdown>
          </Button>
        ...
        <form onSubmit={handleSubmit}>
          <div className={styles.templateType}>
            {selectedTemplateType && <span>
                Mal i bruk: <i>{EVENT_CONSTANTS[selectedTemplateType]}</i>
              </span>}
          </div>

          {survey.templateType ? <h2 style={{
          color: 'red'
        }}>
          ...
          justifyContent: 'space-between'
              ...
              <Field placeholder="Velg arrangement" label="Arrangement" autoFocus={autoFocus} name="event" component={SelectInput.AutocompleteField} className={styles.editEvent} filter={['events.event']} />

              <Field label="Aktiveringstidspunkt" name="activeFrom" component={DatePicker.Field} />
            </div>}
          <FieldArray name="questions" component={this.renderQuestions} rerenderOnEveryChange={true} />
...
export const hasOptions = (data: Record<string, any>) => {
  ...
  data.questions?.forEach((element, i) => {
    if (!['multiple_choice', 'single_choice'].includes(element.questionType)) return;
...
const onSubmit = (formContent: Record<string, any>, dispatch, props: Props) => {
  ...
  const cleanQuestions = questions.map((q, i) => {
    ...
    if (question.questionType === QuestionTypes('text')) {
      ...
      question.options = question.options.filter(option => option.optionText !== '');
    }
