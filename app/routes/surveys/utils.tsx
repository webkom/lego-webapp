...
export const TokenNavigation = ({
  ...
  actionGrant?: ActionGrant;
    ...
    {actionGrant.includes('EDIT') && <NavigationLink to={`/surveys/${surveyId}/submissions/summary`}>
        Adminversjon
      </NavigationLink>}
  </NavigationTab>;
export const defaultActiveFrom = (hours: number, minutes: number) => moment().startOf('day').add({
  day: 1,
  hours,
  minutes
}).toISOString();
export const CHART_COLORS = ['var(--lego-red)', 'var(--color-blue-4)', 'var(--color-orange-3)', 'var(--color-green-5)', 'var(--lego-chart-purple)', 'var(--lego-chart-yellow)', 'var(--lego-chart-green)', '#ff87eb', 'var(--color-black)'];
export const getCsvUrl = (surveyId: string) => `${config.serverUrl}/surveys/${surveyId}/csv/`;
export const QuestionTypeOption = ({
  ...
  cursor: 'pointer',
  backgroundColor: props.isSelected ? 'var(--color-almost-white-1)' : props.isFocused ? 'var(--color-almost-white-5)' : 'var(--color-white)'
}} className={cx(styles.dropdownOption, styles.dropdown)} onMouseDown={event => {
  props.onSelect && props.onSelect(option, event);
}} onMouseEnter={event => props.onFocus && props.onFocus(option, event)} onMouseMove={event => {
...
export const QuestionTypeValue = ({
  ...
  option,
  ...props
}: any) => <div className={cx(styles.dropdownSelected, styles.dropdown)} onMouseDown={event => {
  props.onSelect && props.onSelect(props.option, event);
}} onMouseEnter={event => props.onFocus && props.onFocus(props.option, event)} onMouseMove={event => {
  if (props.isFocused) return;
