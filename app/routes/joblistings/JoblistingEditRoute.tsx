...
import { fetchJoblisting, editJoblisting, deleteJoblisting } from "app/actions/JoblistingActions";
  ...
  return {
    ...
    initialValues: { ...joblisting,
      ...
      fromYear: yearValues.find(({
        value
      }) => value === joblisting.fromYear),
      toYear: yearValues.find(({
        value
      }) => value === joblisting.toYear),
...
}, dispatch) => dispatch(fetchJoblisting(joblistingId))), connect(mapStateToProps, mapDispatchToProps), loadingIndicator(['company.value']))(JoblistingEditor);