...
import { Content, ContentSection, ContentMain, ContentSidebar, ContentHeader } from "app/components/Content";
...
const JoblistingDetail = ({
  ...
  const companyLink = <Link to={`/companies/${joblisting.company.id}`}>
      {joblisting.company.name}
    </Link>;
  const deadline = <strong>
      <Time time={joblisting.deadline} format="ll HH:mm" />
    </strong>;
  const createdAt = <strong>
      <Time time={joblisting.createdAt} format="ll HH:mm" />
    </strong>;
  const canEdit = actionGrant.includes('edit');
  const canDelete = actionGrant.includes('delete');
  return <Content banner={joblisting.company.logo} youtubeUrl={joblisting.youtubeUrl}>
      ...
      <ContentSection>
        ...
        <ContentSidebar>
          ...
          {joblisting.applicationUrl && <a href={joblisting.applicationUrl} style={{
          marginTop: '10px'
        }} target="_blank" rel="noreferrer">
              <strong>SØK HER</strong>
            </a>}
          {(joblisting.responsible || joblisting.contactMail) && <div style={{
          marginTop: '10px'
        }}>
              <h3>Kontaktinfo</h3>
              {joblisting.contactMail && <div>
                  <InfoList items={[{
              key: 'Epost',
              value: joblisting.contactMail
            }]} />
                  {joblisting.responsible && <div style={{
              marginTop: '10px'
            }}>
          ...
          {(canEdit || canDelete) && <Flex column style={{
          marginTop: '10px'
        }}>
              <h3>Admin</h3>
              {canEdit && <Link to={`/joblistings/${joblisting.id}/edit`}>
                  <Button>
