...
function filterJoblistings(joblistings, grades, jobTypes, workplaces) {
  return joblistings.filter(joblisting => {
    const gradeBoolean = grades.length === 0 || grades.find(grade => joblisting.fromYear <= Number(grade) && joblisting.toYear >= Number(grade));
    const jobTypesBoolean = jobTypes.length === 0 || jobTypes.find(jobType => jobType === joblisting.jobType);
    const workplacesBoolean = workplaces.length === 0 || joblisting.workplaces.some(workplace => workplaces.includes(workplace.town)) || workplaces.includes('Annet') && joblisting.workplaces.some(workplace => !['Oslo', 'Trondheim', 'Bergen', 'Tromsø'].includes(workplace.town)) || workplaces.includes('Annet') && joblisting.workplaces.length === 0;
...
const mapStateToProps = (state, props) => {
  ...
  const filterWorkplaces = search.workplaces ? search.workplaces.split(',') : [];
  const filteredJoblistings = filterJoblistings(joblistings, filterGrade, filterJobType, filterWorkplaces);
...
export default compose(prepare((props, dispatch) => dispatch(fetchAll())), connect(mapStateToProps, mapDispatchToProps))(JoblistingPage);