import { selectMostProminentStatus, sortStatusesByProminence, indexToSemester } from "./utils";
...
const sortByAttribute = attribute => ascending => (a, b) => {
  ...
  if (a[attribute].fullName) {
    return ascending ? a[attribute].fullName.localeCompare(b[attribute].fullName) : b[attribute].fullName.localeCompare(a[attribute].fullName);
  }

  return ascending ? a[attribute].localeCompare(b[attribute]) : b[attribute].localeCompare(a[attribute]);
};
...
const sortByContactStatus = (index, startYear, startSem) => ascending => (a, b) => {
  ...
  const semesterA = a.semesterStatuses.find(obj => obj.year === companySemester.year && obj.semester === companySemester.semester);
  const statusA = semesterA ? selectMostProminentStatus(semesterA.contactedStatus) : 'not_contacted';
  const semesterB = b.semesterStatuses.find(obj => obj.year === companySemester.year && obj.semester === companySemester.semester);
  const statusB = semesterB ? selectMostProminentStatus(semesterB.contactedStatus) : 'not_contacted';
...
const sortCompanies = (companies, query, startYear, startSem) => {
  ...
  const sortTypeName = Object.keys(sortTypeToFunction).find(sortTypeName => sortTypeName === sortType);
  const sortFunction = sortTypeName ? sortTypeToFunction[sortTypeName](ascending) : sortTypeToFunction.name(true);
