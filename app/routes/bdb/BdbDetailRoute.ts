...
import { fetchAdmin, deleteSemesterStatus, deleteCompanyContact, fetchSemesters, editSemesterStatus, fetchEventsForCompany, editCompany, deleteCompany } from "app/actions/CompanyActions";
import BdbDetail from "./components/BdbDetail";
import { compose, bindActionCreators } from "redux";
import { selectCompanyById, selectEventsForCompany, selectCommentsForCompany } from "app/reducers/companies";
...
}, dispatch) => Promise.all([dispatch(fetchSemesters()).then(() => dispatch(fetchAdmin(companyId))), dispatch(fetchEventsForCompany({
...
export default compose(replaceUnlessLoggedIn(LoginPage), prepare(loadData, ['match.params.companyId']), connect(mapStateToProps, mapDispatchToProps))(BdbDetail);