...
export default class CompanySingleRow extends Component<Props> {
  semesterElement = (index: number) => {
    ...
    return (company.semesterStatuses || []).find(status => status.year === result.year && status.semester === result.semester) || {
  ...
  render() {
    ...
    const semesters = [this.semesterElement(0), this.semesterElement(1), this.semesterElement(2)].map((status, i) => <SemesterStatus key={i} semIndex={i} semesterStatus={status} editChangedStatuses={editChangedStatuses} companyId={company.id} />);
    return <tr>
        <td>
        ...
        {semesters}
        <td>
          {company.studentContact && (typeof company.studentContact === 'string' ? company.studentContact : <Link to={`/users/${company.studentContact.username}`}>
