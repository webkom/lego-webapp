...
const CompanyList = ({
  ...
  const findSortLink = (sortType: string) => {
  ...
  const showOrHideSortIcon = (sortType: string) => {
    const ascending = query.ascending === 'true';
  ...
  const headers = HEADER_ITEMS.map((item, i) => (
    <th key={i}>
      <Link to={findSortLink(item.sortLink)}>
        <div className={styles.title}>{item.title}</div>

        <div className={styles[showOrHideSortIcon(item.sortLink)]}>
