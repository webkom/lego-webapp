...
import { Content, ContentSection, ContentSidebar, ContentMain } from "app/components/Content";
import type { MeetingInvitationEntity, MeetingInvitationStatus } from "app/reducers/meetingInvitations";
...
type Props = {
  ...
  setInvitationStatus: (meetingId: number, status: MeetingInvitationStatus, user: UserEntity) => Promise<any>;
...
class MeetingDetails extends Component<Props> {
  ...
  sortInvitations = () => {
    ...
    return (Object.keys(statuses).map(invitationStatus => ({
      ...
      registrations: meetingInvitations.filter(invite => invite.status === invitationStatus)
    })) as Array<{
      name: string;
      capacity: number;
  ...
  attendanceButtons = (statusMe: string | null | undefined, startTime: Dateish) => statusMe && moment(startTime) > moment() && <li className={styles.statusButtons}>
        <Button success onClick={this.acceptInvitation} disabled={statusMe === statuses.ATTENDING}>
          Delta
        </Button>
        <Button dark onClick={this.rejectInvitation} disabled={statusMe === statuses.NOT_ATTENDING}>
          Avslå
        </Button>
      </li>;
  ...
  render() {
    ...
    return <div>
        ...
        <Content>
          <NavigationTab title={meeting.title} className={styles.detailTitle} details={<Time style={{
          color: 'grey'
        }} time={meeting.startTime} format="ll [-] HH:mm" />} back={{
          label: 'Mine møter',
          path: '/meetings'
        }}>
            {canEdit && <NavigationLink to={`/meetings/${meeting.id}/edit`}>
                Rediger
              </NavigationLink>}
          </NavigationTab>
          ...
          <ContentSection>
            <ContentMain>
              {meeting.description && <div>{urlifyString(meeting.description)}</div>}
              <h2>Referat</h2>
            ...
            <ContentSidebar>
              ...
              padding: 0
                ...
                <ul>
                  ...
                  {meeting.mazemapPoi && <MazemapEmbed mazemapPoi={meeting.mazemapPoi} />}
                  <li>
          ...
          <ContentSection>
            <ContentMain>
              {meeting.contentTarget && <CommentView user={currentUser} contentTarget={meeting.contentTarget} loggedIn={loggedIn} comments={comments} deleteComment={deleteComment} />}
            </ContentMain>
          </ContentSection>
        </Content>
      </div>;
