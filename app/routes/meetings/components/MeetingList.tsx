...
function MeetingListItem({
  ...
  return <div style={{
    borderColor: isDone ? 'gray' : 'red'
  }} className={styles.meetingItem}>
      <div>
        <Link to={`/meetings/${meeting.id}`}>
          <h3 className={styles.meetingItemTitle}>
            {meeting.title}
            {username === meeting.createdBy && <Pill style={{
            marginLeft: 10
          }}>Eier</Pill>}
            {username === meeting.reportAuthor && <Pill style={{
            marginLeft: 10
          }}>Referent</Pill>}
          </h3>
        </Link>
...
const MeetingListView = ({
  ...
  currentUser: UserEntity;
    ...
    {sections.map((item, key) => <div key={key}>
        <h2 className={styles.heading}>{item.title}</h2>
        {item.meetings.map((item, key) => <MeetingListItem key={key} username={currentUser.username} meeting={item} />)}
      </div>)}
    {!sections.length && <h2 style={{
    textAlign: 'center',
    marginBottom: 10
  }}>
        Ingen møter å vise
...
export default class MeetingList extends Component<Props> {
  render() {
    ...
    return <Content>
        <Toolbar />
        {!meetingSections || loading ? <LoadingIndicator loading /> : <MeetingListView currentUser={currentUser} sections={meetingSections} />}
