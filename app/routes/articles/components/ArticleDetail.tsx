...
import { useArticleByIdOrSlug } from 'app/actions/ArticleActions';
...
const ArticleDetail = () => {
  ...
  const articleRequest = useArticleByIdOrSlug(articleIdOrSlug);
  ...
  return when(articleRequest, {
    pending: () => <LoadingPage cover loading />,
    data: (article) => (
      <Page
        title={article.title}
        cover={
