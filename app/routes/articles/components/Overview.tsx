...
const OverviewItem = ({
  ...
  article: ArticleEntity;
    ...
    <span className={styles.itemInfo}>
      {article.author.id && <span>
          <Link to={`/users/${article.author.username}`} className={styles.overviewAuthor}>
            {' '}
  ...
  render() {
    ...
    return <Content>
        ...
        <NavigationTab title="Artikler">
          {actionGrant.includes('create') && <NavigationLink to="/articles/new">Ny artikkel</NavigationLink>}
        </NavigationTab>
        <Tags>
          {this.props.tags.map(tag => {
          const isSelected = query && query.tag === tag.tag;
          return <Tag tag={tag.tag} key={tag.tag} color="blue" active={isSelected} link={isSelected ? '/articles/' : `/articles?tag=${tag.tag}`} />;
        })}
          <Tag tag="Vis alle tags ..." key="viewmore" link="/tags/" color="gray" />
        </Tags>
        <section className={styles.frontpage}>
          <Paginator infiniteScroll={true} hasMore={this.props.hasMore} fetching={this.props.fetching} fetchNext={() => {
          this.props.fetchAll({
            query,
            next: true
          });
