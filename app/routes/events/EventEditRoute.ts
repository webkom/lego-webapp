...
import { selectEventById, selectPoolsWithRegistrationsForEvent, selectRegistrationsFromPools, selectWaitingRegistrationsForEvent } from "app/reducers/events";
...
const mapStateToProps = (state, props) => {
  ...
  return {
    initialValues: { ...event,
      mergeTime: event.mergeTime ? event.mergeTime : time({
        hours: 12
      }),
      ...
      eventStatusType: event.eventStatusType && transformEventStatusType(event.eventStatusType),
      mazemapPoi: event.mazemapPoi && {
        label: event.location,
        //if mazemapPoi has a value, location will be its displayname
        value: event.mazemapPoi
      },
      separateDeadlines: event.registrationDeadlineHours !== event.unregistrationDeadlineHours,
...
export default compose(replaceUnlessLoggedIn(LoginPage), prepare(({
  match: {
    params: {
      eventId
    }
  }
}, dispatch) => dispatch(fetchEvent(eventId))), connect(mapStateToProps, mapDispatchToProps), loadingIndicator(['event.title']))(EventEditor);