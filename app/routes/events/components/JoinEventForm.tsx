...
import { paymentSuccess, paymentManual, sumPenalties, penaltyHours, registrationIsClosed, getEventSemesterFromStartTime, allConsentsAnswered, toReadableSemester } from "../utils";
import { selectUserByUsername } from "app/reducers/users";
import { selectPenaltyByUserId } from "app/reducers/penalties";
import type { User, EventRegistration, EventRegistrationStatus } from "app/models";
...
const SubmitButton = ({
  ...
  if (type === 'register') {
    return <Button className={styles.registrationBtn} onClick={onSubmit} disabled={disabled}>
        {title}
      </Button>;
  }

  const message = <Flex column>
      <span>Er du sikker på at du vil avregistrere deg?</span>
      {showPenaltyNotice && <b>NB: Avregistrering medfører én prikk</b>}
    </Flex>;
  return <ConfirmModalWithParent title="Avregistrer" message={message} onConfirm={() => {
    onSubmit && onSubmit();
  ...
  reg_status?: EventRegistrationStatus;
    ...
    <p>
      ...
      <Tooltip content={<span>
            ...
            <a target="blank" rel="noopener noreferrer" href="https://abakus-ntnu.slack.com/archives/C0L63DZRU">
              #webkom
            </a>{' '}
            på slack for eventuelle oppdateringer.
          </span>} renderDirection="center">
...
const PaymentForm = ({
  ...
  width: '100%'
    ...
    <PaymentRequestForm paymentError={registration.paymentError} createPaymentIntent={createPaymentIntent} event={event} currentUser={currentUser} paymentStatus={registration.paymentStatus} clientSecret={registration.clientSecret} />
  </div>;
...
const JoinEventForm = (props: Props) => {
  ...
  const disabledButton = !registration ? isInvalid || isPristine || submitting : false;
  const disabledForUser = !formOpen && !event.activationTime && !registration;
  const showPenaltyNotice = Boolean(event.heedPenalties && moment().isAfter(event.unregistrationDeadline) && registration && registration.pool);
  const registrationPending = pendingRegistration?.status === 'PENDING_REGISTER' || pendingRegistration?.status === 'PENDING_UNREGISTER';
  const showCaptcha = !submitting && !registrationPending && !registration && captchaOpen && event.useCaptcha;
  const showStripe = event.useStripe && event.isPriced && event.price > 0 && registration && registration.pool && ![paymentManual, paymentSuccess].includes(registration.paymentStatus);
  const [registrationPendingDelayed, setRegistrationPendingDelayed] = useState(false);
  const eventSemester = getEventSemesterFromStartTime(event.startTime);
  const hasRegisteredConsentForSemester = allConsentsAnswered(event.photoConsents);
  const hasRegisteredConsentIfRequired = event.useConsent ? hasRegisteredConsentForSemester : true;
  useEffect(() => {
    const timer = setTimeout(() => setRegistrationPendingDelayed(registrationPending), registrationPendingDelayed && !registrationPending ? 0 : 1000);
  ...
  if (registrationIsClosed(event)) {
    return <>
        {!formOpen && registration && showStripe && <PaymentForm createPaymentIntent={createPaymentIntent} event={event} currentUser={currentUser} registration={registration} />}
      </>;
  }
  ...
  const registrationMessage = event => {
    switch (event.eventStatusType) {
      ...
      case 'TBA':
        return <div>Påmelding til dette arrangementet er ikke bestemt enda</div>;
  ...
  return <>
      ...
      <Flex column>
        {['OPEN', 'TBA'].includes(event.eventStatusType) ? registrationMessage(event) : <>
            {!formOpen && event.activationTime && <div>
                {new Date(event.activationTime) < new Date() ? 'Åpnet ' : 'Åpner '}
                <Time time={event.activationTime} format="nowToTimeInWords" />
              </div>}
            {disabledForUser && <div>Du kan ikke melde deg på dette arrangementet.</div>}
            {sumPenalties(penalties) > 0 && event.heedPenalties && <div className={styles.eventWarning}>
                <p>
                  NB!{' '}
                  {sumPenalties(penalties) > 2 ? `Du blir lagt rett på venteliste hvis du melder deg på` : `Påmeldingen din er forskjøvet
            ...
            {!disabledForUser && event.useContactTracing && !currentUser.phoneNumber && <div className={styles.eventWarning}>
                  <p>NB!</p>
                  <p>
            ...
            {!disabledForUser && event.useConsent && !hasRegisteredConsentForSemester && <div className={styles.eventWarning}>
                  <p>NB!</p>
                  <p>
            ...
            {formOpen && hasRegisteredConsentIfRequired && (event.useContactTracing ? currentUser.phoneNumber : true) && <Flex column>
                  <Form onSubmit={submitWithType(handleSubmit, feedbackName, registrationType)}>
                    {showCaptcha && <Field name="captchaResponse" fieldStyle={{
              width: 304
            }} component={Captcha.Field} />}
                    {event.activationTime && registrationOpensIn && <Flex alignItems="center">
                    ...
                    {buttonOpen && !submitting && !registrationPending && <>
                        <Flex alignItems="center" justifyContent="space-between">
                          <SubmitButton disabled={disabledButton} onSubmit={submitWithType(handleSubmit, feedbackName, registrationType)} type={registrationType} title={title || joinTitle} showPenaltyNotice={showPenaltyNotice} />
                        </Flex>
                        {!registration && <SpotsLeft activeCapacity={event.activeCapacity} spotsLeft={event.spotsLeft} />}
                      </>}
                    {submitting || registrationPending && !registrationPendingDelayed && <LoadingIndicator loading loadingStyle={{
              margin: '5px auto'
            }} />}
                    {registrationPendingDelayed && <RegistrationPending reg_status={pendingRegistration?.status} />}
                  </Form>
            ...
            marginBottom: '20px'
                    ...
                    <Field id={feedbackName} placeholder="Melding til arrangør" name={feedbackName} component={TextInput.Field} labelClassName={styles.feedbackLabel} className={styles.feedbackText} fieldClassName={styles.feedbackField} rows={1} />
                    {registration && <Button type="button" onClick={submitWithType(handleSubmit, feedbackName, 'feedback')} success className={styles.feedbackUpdateButton} disabled={pristine}>
                        Oppdater
                      </Button>}
                  </Flex>
                  {registration && showStripeDelayed && <PaymentForm event={event} createPaymentIntent={createPaymentIntent} currentUser={currentUser} registration={registration} />}
                </Flex>}
          </>}
      </Flex>
    </>;
...
function mapStateToProps(state, {
  ...
  const user = state.auth ? selectUserByUsername(state, {
    username: state.auth.username
  }) : null;
  const penalties = user ? selectPenaltyByUserId(state, {
    userId: user.id
  }) : [];
  return {
    penalties
...
export default compose(connect(mapStateToProps, null), withCountdown, reduxForm({
  form: 'joinEvent',
