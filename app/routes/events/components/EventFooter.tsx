...
const getIcalUrl = (icalToken, icalType) => `${config.serverUrl}/calendar-ical/${icalType}/?token=${icalToken}`;
...
const EventFooter = ({
  icalToken
}: Props) => <div className={styles.eventFooter}>
    {icalToken && <p className={styles.section}>
    ...
    <div className={styles.eventFooterSections}>
      {icalToken && <div className={styles.section}>
          <h3>Google kalender</h3>
          <ul>
            {icalTypes.map((type, key) => <li key={key}>
      ...
      <div className={styles.section}>
        <h3>Fargekoder</h3>
        <ul>
          {Object.keys(EVENT_CONSTANTS).map((e, i) => <li key={i}>
