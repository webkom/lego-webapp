...
describe('components', () => {
  describe('CommentView', () => {
    it('should render a comment tree', () => {
      ...
      expect(wrapper.find(CommentTree).length).to.deep.equal(1);
    });
    ...
    it('should pass the comment tree a tree structure', () => {
      ...
      expect(first.children.length).to.deep.equal(0);
      expect(second.children.length).to.deep.equal(1);
      expect(second.children[0].parent).to.deep.equal(second.id);
    });
  });
});
