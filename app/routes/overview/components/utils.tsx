...
export const renderMeta = (item: (Event | Article) & {
  ...
  return <span className={styles.itemInfo}>
    ...
    time={isEvent ? item.startTime : item.createdAt} format={moment().year() === moment(item.startTime).year() ? 'DD. MMM HH:mm' : 'DD. MMM YYYY HH:mm'} />

      {item.location !== '-' && isEvent && <span>
          <span> • </span>
      ...
      {item.tags?.length > 0 && <Tags className={styles.tagline}>
          {item.tags.slice(0, 3).map(tag => <Tag tag={tag} key={tag} />)}
        </Tags>}
    </span>;
};