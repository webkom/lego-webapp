...
const FeedItem = (props: {
  ...
  if (renders) {
    return <Link to={renders.getURL(props.activity)}>
        <li className={styles.item}>
          ...
          <div className={styles.wordBreak}>
            ...
            <Time time={props.activity.updatedAt} wordsAgo style={{
            margin: '0',
            display: 'block'
          }} />
          </div>
...
const Feed = (props: Props) => {
  return <div className={styles.root}>
      <div className={styles.header}>
        <h2 className="u-ui-heading" style={{
        padding: '0',
        margin: '0'
      }}>
          Oppdateringer
      ...
      <div className={styles.content}>
        {props.feedItems.length > 0 ? <ul>
            {props.feedItems.map(activity => <ErrorBoundary hidden key={activity.id}>
                <FeedItem activity={activity} />
              </ErrorBoundary>)}
          </ul> : <EmptyState className={styles.noActivities} icon="book-outline" size={40}>
