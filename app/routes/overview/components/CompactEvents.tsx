...
export default class CompactEvents extends Component<Props> {
  render() {
    ...
    const mapEvents = eventTypes => {
      return events.filter(event => event.endTime.isAfter() && eventTypes.includes(event.eventType)).slice(0, 5).map((event, key) => <li key={key}>
            <span style={{
          ...
          marginRight: '10px'
            ...
            {event.pinned && <Tooltip content="Dette arrangementet er festet til forsiden">
                <i className="fa fa-thumb-tack" style={{
            transform: 'rotate(-20deg)',
            marginRight: '4px',
            color: '#BE1600'
          }} />
              </Tooltip>}
            <Time format="dd D.MM" time={event.startTime} style={{
          flex: '0 1 0'
        }} />
          </li>);
    };

    const presentations = mapEvents(['company_presentation', 'lunch_presentation', 'alternative_presentation', 'course', 'breakfast_talk']);
    const leftEvents = presentations.length > 0 ? presentations : ['Ingen presentasjoner'];
