...
export default compose( // Feed not in use
// prepare(({ loggedIn }, dispatch) =>
//   loggedIn ? dispatch(fetchPersonalFeed()) : Promise.resolve()
// ),
connect(mapStateToProps, mapDispatchToProps), prepare(({
  shouldFetchQuote,
  loggedIn
}, dispatch) => Promise.all([loggedIn && shouldFetchQuote && dispatch(fetchRandomQuote()), dispatch(fetchReadmes(loggedIn ? 4 : 1))]), [], {
  awaitOnSsr: false
}), prepare(({
  loggedIn
}, dispatch) => dispatch(fetchData())), replaceUnlessLoggedIn(PublicFrontpage))(Overview);