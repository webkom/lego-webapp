...
import { companySchema, companySemesterSchema, eventSchema, joblistingsSchema } from "app/reducers";
...
export const fetchEventsForCompany = ({
  ...
  const endpoint = loadNextPage ? getState().events.pagination[queryString].nextPage : `/events/${queryString}`;
...
export function deleteSemesterStatus(companyId: number, semesterStatusId: number): Thunk<any> {
  return callAPI({
...
export function deleteCompanyContact(companyId: number, companyContactId: number): Thunk<any> {
  return callAPI({
...
export function fetchSemesters(queries: Record<string, (string | null | undefined) | (number | null | undefined)> = {}): Thunk<any> {
  return callAPI({
