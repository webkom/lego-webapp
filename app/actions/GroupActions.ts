...
export function editGroup(group: Record<string, any>): Thunk<any> {
  return dispatch => dispatch(callAPI({
    ...
    meta: {
      group,
      errorMessage: group.type === 'interesse' ? 'Endring av interessegruppe feilet' : 'Oppdatering av gruppe feilet',
      successMessage: group.type === 'interesse' ? 'Endring av interessegruppe fullført' : 'Oppdatering av gruppe fullført'
    }
  })).then(() => group.type === 'interesse' ? dispatch(push(`/interest-groups/${group.id}`)) : null);
}
export function joinGroup(groupId: number, user: Record<string, any>, role: string = 'member'): Thunk<any> {
  return dispatch => dispatch(callAPI({
...
export function leaveGroup(membership: Record<string, any>, groupId: number): Thunk<any> {
  return dispatch => {
...
export function fetchAllMemberships(groupId: number, descendants: boolean = false): Thunk<any> {
  return dispatch => {
    return dispatch(fetchMembershipsPagination({
      descendants,
      groupId,
      next: true
    })).then(res => res.payload.next && dispatch(fetchAllMemberships(groupId, descendants)));
  };
}
export function fetchMemberships(groupId: number, descendants: boolean = false, query: Record<string, any> = {}): Thunk<any> {
...
export function createGroup(group: Record<string, any>): Thunk<any> {
  return dispatch => {
    ...
    return dispatch(callAPI({
      ...
      meta: {
        group,
        errorMessage: group.type === 'interesse' ? 'Opprettelse av interessegruppe feilet' : 'Opprettelse av gruppe feilet',
        successMessage: group.type === 'interesse' ? 'Opprettelse av interessegruppe fullført' : 'Opprettelse av gruppe fullført'
      }
    })).then(action => {
...
export function removeGroup(id: string, group: Record<string, any>): Thunk<any> {
  return dispatch => dispatch(callAPI({
    types: Group.REMOVE,
    endpoint: `/groups/${id}/`,
    method: 'DELETE',
    meta: {
      id,
      errorMessage: group.type === 'interesse' ? 'Sletting av interessegruppe feilet' : 'Sletting av gruppe feilet',
      successMessage: group.type === 'interesse' ? 'Sletting av interessegruppe fullført' : 'Sletting av gruppe fullført'
    }
